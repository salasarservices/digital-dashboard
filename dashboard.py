...
if pdf_report_btn:
    # Executive KPIs
    executive_kpis = [
        {"title": "Website Clicks", "value": f"{gsc_clicks:,}", "delta_text": f"{gsc_clicks_delta:+.2f}%"},
        {"title": "Impressions", "value": f"{gsc_impressions:,}", "delta_text": f"{gsc_impr_delta:+.2f}%"},
        {"title": "Average CTR", "value": f"{gsc_ctr*100:.2f}%", "delta_text": f"{gsc_ctr_delta:+.2f}%"},
    ]
    ...
    sections = [
        {
            ...
            "chart_fig": None,
            "table_df": None,
            "notes": "Data source: Google Search Console & GA4"
        },
        {
            "title": "Top Content",
            "kpis": [],
            "chart_fig": None,
            "table_df": pd.DataFrame(top_content_data) if top_content_data else pd.DataFrame(),
            "notes": "Top pages ranked by clicks"
        },
        {
            "title": "Website Analytics",
            "kpis": [
                {"title": "Total Users", "value": f"{cur:,}", "delta_text": f"{delta:+.2f}%"},
                {"title": "Sessions", "value": f"{total:,}", "delta_text": f"{delta2:+.2f}%"},
                {"title": "Organic Clicks", "value": f"{clicks:,}", "delta_text": f"{delta3:+.2f}%"}
            ],
            "chart_fig": None,
            "table_df": pd.DataFrame(traf) if traf else pd.DataFrame(),
            "notes": "Sessions by channel"
        },
        {
            "title": "Leads",
            "kpis": [
                {"title": "Total Leads", "value": f"{total_leads}", "delta_text": ""},
                {"title": "Interested", "value": f"{interested_count}", "delta_text": ""},
                {"title": "Closed", "value": f"{closed_count}", "delta_text": ""},
            ],
            "chart_fig": None,
            "table_df": df if not df.empty else None,
            "notes": "Leads data from MongoDB"
        }
    ]

    logo_url = "https://www.salasarservices.com/assets/Frontend/images/logo-black.png"
    pdf_io = generate_pdf_report_fpdf(
        reporting_period=format_month_year(sd),
        logo_url=logo_url,
        executive_kpis=executive_kpis,
        sections=sections,
        footer_text="Generated by Salasar Services Digital Marketing Reporting Dashboard"
    )

    st.sidebar.download_button(
        label="Download PDF (Improved Layout)",
        data=pdf_io,
        file_name=f"Salasar-Services-Report-{date.today().isoformat()}.pdf",
